...
describe("resolveAgentVariant", () => {
  test("returns undefined when agent name missing", () => {
    // given
    const config = {} as OhMyGeminiConfig
  ...
  test("returns category variant when agent uses category", () => {
    ...
    const config = {
      ...
      categories: {
        ultrabrain: { model: "google/gemini-3-pro-preview", variant: "xhigh" },
      },
    } as OhMyGeminiConfig
...
describe("resolveVariantForModel", () => {
  test("returns agent override variant when configured", () => {
    // given - use a model in sisyphus chain (gemini-3-pro-preview has default variant "max")
    ...
    const model = { providerID: "google", modelID: "gemini-3-pro-preview" }
  ...
  test("returns correct variant for google provider", () => {
    // given
    const config = {} as OhMyGeminiConfig
    const model = { providerID: "google", modelID: "gemini-3-pro-preview" }
  ...
  test("returns correct variant for google provider (hephaestus agent)", () => {
    // #given hephaestus has google/gemini-3-pro-preview with variant "max" in its chain
    const config = {} as OhMyGeminiConfig
    const model = { providerID: "google", modelID: "gemini-3-pro-preview" }
  ...
  test("returns correct variant for oracle agent with google", () => {
    // given
    const config = {} as OhMyGeminiConfig
    const model = { providerID: "google", modelID: "gemini-3-pro-preview" }
