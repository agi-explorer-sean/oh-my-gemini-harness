...
66 cross-cutting utilities. Example: `{ log, deepMerge }` from `@shared` barrel.
...
├── logger.ts              # File-based logging (/tmp/oh-my-gemini.log) - 53 imports
...
├── gemini-config-dir.ts # ~/.config/gemini resolution (143 lines) - 9 imports
├── claude-config-dir.ts   # ~/.claude resolution - 9 imports
├── migration.ts           # Legacy config migration logic (231 lines)
├── gemini-version.ts    # Semantic version comparison
...
| gemini-config-dir.ts | 9 | Path resolution |
...
| Task | Utility |
|------|---------|
| Path Resolution | `getGeminiConfigDir()`, `getDataPath()` |
...
| Version Gating | `isGeminiVersionAtLeast(version)` |
...
- **Manual Version Check**: Use `gemini-version.ts` for semver safety
