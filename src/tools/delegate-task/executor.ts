...
import {discoverSkills} from '../../features/gemini-skill-loader';
import {resolveMultipleSkillsAsync} from '../../features/gemini-skill-loader/skill-content';
import {
...
export interface ExecutorContext {
  ...
  /** When true, session creation is unavailable (MCP server mode). */
  isMcpMode?: boolean;
}
...
export async function executeBackgroundContinuation(
  ...
  if (
    executorCtx.isMcpMode ||
    !client.session.create ||
    typeof client.session.create !== 'function'
  ) {
    return `CONTINUATION UNAVAILABLE

Programmatic session continuation is blocked in this environment. Use @agent mentions directly.`;
  }
...
export async function executeSyncContinuation(
  ...
  if (
    ...
    return `SYNC CONTINUATION UNAVAILABLE
    
Programmatic session polling is blocked in this environment. Use @agent mentions directly.`;
  }
...
const SUBPROCESS_NOISE_PATTERNS =
  /^(Loading extension|Error during discovery|Failed to compile schema|type must be JSONType|YOLO mode is enabled|Expanding hook command|Hook execution|Created execution plan|\[ExtensionManager\]|MCP issues detected)/;

// The "MCP issues detected" warning is sometimes printed without a trailing
// newline, gluing it to the first line of real content. Strip it as a prefix.
const MCP_STATUS_PREFIX =
  /^MCP issues detected\. Run \/mcp list for status\./;

export function findGeminiPath(): string | null {
  const sarArgv0 = process.env.SAR_ARGV0;
  if (sarArgv0 && existsSync(sarArgv0)) return sarArgv0;

  try {
    const which = Bun.which('gemini');
...
/**
 * Execute a delegation task by spawning a separate gemini process.
 *
 * This creates a truly isolated session with its own context window,
 * preventing context rot in multi-delegation workflows. Each subprocess
 * loads agents from `agents/*.md` and extensions independently.
 */
export async function executeSubprocessTask(
  args: DelegateTaskArgs,
  agentToUse: string,
  directory: string,
  systemContent?: string,
): Promise<string> {
  // Prevent recursive self-delegation: if OMG_PARENT_AGENT matches the
  // target agent, we're already running as that agent in a parent process.
  const parentAgent = process.env.OMG_PARENT_AGENT;
  if (parentAgent && parentAgent.toLowerCase() === agentToUse.toLowerCase()) {
    log(
      `[delegate_task] Blocked recursive delegation: already running as ${agentToUse} (env OMG_PARENT_AGENT=${parentAgent})`,
    );
    return `You are already running as the ${agentToUse} agent. Use your available tools directly (web_search, web_fetch, grep_search, read_file, etc.) instead of delegating to yourself.`;
  }

  const geminiPath = findGeminiPath();
  if (!geminiPath) {
    return `SUBPROCESS DELEGATION FAILED

Could not find the gemini binary. Tried:
1. SAR_ARGV0 environment variable
2. Bun.which('gemini')

Fallback: Use @${agentToUse} mention directly in the conversation.`;
  }

  // Build prompt: @agent prefix for agent selection + optional system context
  const parts: string[] = [];
  if (agentToUse) parts.push(`@${agentToUse}`);
  if (systemContent) parts.push(`[Context]\n${systemContent}\n\n---\n`);

  // Inject tool restriction instructions for agents with denied tools.
  // In subprocess mode, we can't use session.prompt(tools: {...}) to deny
  // tools programmatically, so we inject instructions into the prompt.
  ...
  if (deniedTools.length > 0) {
    parts.push(
      `<tool-restrictions>\nDo NOT use these tools: ${deniedTools.join(', ')}. They are disabled for this agent. Use your other available tools directly.\n</tool-restrictions>\n`,
    );
  }
  ...
  log('[delegate_task] subprocess delegation', {
    geminiPath,
    agentToUse,
  ...
  const toastManager = getTaskToastManager();
  const taskId = `subprocess_${agentToUse}_${Date.now()}`;
  const startTime = Date.now();

  if (toastManager) {
    toastManager.addTask({
      id: taskId,
      description: `${args.description} (subprocess)`,
      agent: agentToUse,
      isBackground: false,
    });
  }
  ...
  try {
    ...
    // startup (port allocation, etc.).
    const parentProxyAddress = process.env.GOOGLE_GEMINI_BASE_URL;
    const geminiArgs = parentProxyAddress
      ? [geminiPath, `--proxy_address=${parentProxyAddress}`, '--', '-y', '-p', fullPrompt]
      : [geminiPath, '--', '-y', '-p', fullPrompt];
    ...
    const proc = Bun.spawn(geminiArgs, {
      ...
      env: {
        ...process.env,
        GLOG_minloglevel: '2',
        // Pass the agent name to child processes so they can detect
        // self-delegation and avoid recursive subprocess spawning.
        OMG_PARENT_AGENT: agentToUse,
      },
    });

    const timeout = setTimeout(() => {
      log('[delegate_task] subprocess timeout, killing', {
        agentToUse,
    ...
    log('[delegate_task] subprocess completed', {
      agentToUse,
      exitCode,
...
Fallback: Use @${agentToUse} mention directly in the conversation.`;
    ...
    // Filter noise lines from gemini CLI output.
    // First strip the MCP status prefix that may be glued to real content
    // (gemini prints it without a trailing newline), then drop full noise lines.
    const filteredOutput = stdout
      .split('\n')
      .map((line) => line.replace(MCP_STATUS_PREFIX, ''))
      .filter((line) => !SUBPROCESS_NOISE_PATTERNS.test(line.trim()))
      .join('\n')
      .trim();
    ...
    return `Task delegated to ${agentToUse} via isolated subprocess (${durationStr}).
...
export async function executeUnstableAgentTask(
  ...
  if (executorCtx.isMcpMode) {
    return executeSubprocessTask(
      args,
      agentToUse,
...
export async function executeBackgroundTask(
  ...
  if (
    ...
    // Use subprocess delegation as fallback — runs synchronously despite being called "background"
    return executeSubprocessTask(
      args,
      agentToUse,
      executorCtx.directory,
...
export async function executeSyncTask(
  ...
  if (
    ...
    log(
      '[delegate_task] Restricted API detected (isMcpMode=%s). Using subprocess delegation.',
      executorCtx.isMcpMode,
    );
    return executeSubprocessTask(args, agentToUse, directory, systemContent);
  }
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        return formatDetailedError(
          new Error(
            `Agent "${agentToUse}" not found. Make sure the agent is registered in your gemini.json or provided by a plugin.`,
          ),
          {
...
Configure in one of:
1. Gemini: Set "model" in gemini.json
2. Oh-My-Gemini: Set category model in oh-my-gemini.json
