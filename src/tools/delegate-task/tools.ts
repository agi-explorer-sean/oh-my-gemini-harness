...
import {CATEGORY_DESCRIPTIONS, DEFAULT_CATEGORIES} from './constants';
import {
...
Prompts MUST be in English.`;
  ...
  return tool({
    ...
    async execute(args: DelegateTaskArgs, toolContext) {
      ...
      try {
        const geminiConfig = await options.client.config.get();
        systemDefaultModel = (geminiConfig as {data?: {model?: string}})?.data
          ?.model;
      } catch {
