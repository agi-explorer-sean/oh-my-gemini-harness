...
import { clearSkillCache } from "../../features/gemini-skill-loader/skill-content"
...
describe("sisyphus-task", () => {
  ...
  describe("category delegation config validation", () => {
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      expect(result).not.toContain("oh-my-gemini requires a default model")
    }, { timeout: 10000 })
  ...
  test("session_id with background=true should return immediately without waiting", async () => {
    ...
    const mockClient = {
      session: {
        create: async () => ({ data: { id: "ses_bg_continue" } }),
        prompt: async () => ({ data: {} }),
        messages: async () => ({
          data: [],
        }),
  ...
  describe("browserProvider propagation", () => {
    ...
    test("should NOT resolve agent-browser skill when browserProvider is not set", async () => {
      ...
      // when - request non-existent skill
      const result = await tool.execute(
        {
          ...
          category: "ultrabrain",
          run_in_background: false,
          load_skills: ["non-existent-skill-12345"],
        },
        toolContext
      )
      ...
      expect(result).toContain("non-existent-skill-12345")
    })
  })
  ...
  describe("session title and metadata format (Gemini compatibility)", () => {
    test("sync session title follows Gemini format: '{description} (@{agent} subagent)'", async () => {
      // given
      const { createDelegateTask } = require("./tools")
      let createBody: any
      ...
      // then - title should follow Gemini format
