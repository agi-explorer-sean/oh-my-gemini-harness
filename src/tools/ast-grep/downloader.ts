...
function getAstGrepVersion(): string {
  try {
    ...
    const pkgName = "@ast-grep/cli/package.json"
...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[oh-my-gemini] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[oh-my-gemini] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[oh-my-gemini] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[oh-my-gemini] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
