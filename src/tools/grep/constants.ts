...
function getGeminiBundledRg(): string | null {
  ...
  const candidates = [
    // Gemini XDG data path (highest priority - where Gemini installs rg)
    join(getDataDir(), "gemini", "bin", rgName),
...
export function resolveGrepCli(): ResolvedCli {
  ...
  const bundledRg = getGeminiBundledRg()
  if (bundledRg) {
