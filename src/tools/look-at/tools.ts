...
export function createLookAt(ctx: PluginInput): ToolDefinition {
  return tool({
    ...
    async execute(rawArgs: LookAtArgs, toolContext) {
      ...
      // In Gemini CLI mode (no session API), use subprocess delegation
      if ((ctx as any).isMcpMode) {
        const subprocessPrompt = `Analyze the file at ${args.file_path}. Goal: ${args.goal}. Use read_file to read the file contents first, then provide analysis matching the goal.`
        return executeSubprocessTask(
          {
            description: `look_at: ${args.goal.substring(0, 50)}`,
            prompt: subprocessPrompt,
...
- Check provider connections in gemini settings
