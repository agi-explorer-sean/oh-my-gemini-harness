...
import { getGeminiConfigDir, getDataDir } from "../../shared"
...
type ConfigSource = "project" | "user" | "gemini"
...
function getConfigPaths(): { project: string; user: string; gemini: string } {
  const cwd = process.cwd()
  const configDir = getGeminiConfigDir({ binary: "gemini" })
  return {
    project: join(cwd, ".gemini", "oh-my-gemini.json"),
    user: join(configDir, "oh-my-gemini.json"),
    gemini: join(configDir, "gemini.json"),
  }
}
...
function loadAllConfigs(): Map<ConfigSource, ConfigJson> {
  ...
  const gemini = loadJsonFile<ConfigJson>(paths.gemini)
  if (gemini) configs.set("gemini", gemini)
...
function getMergedServers(): ServerWithSource[] {
  ...
  const sources: ConfigSource[] = ["project", "user", "gemini"]
  ...
  return servers.sort((a, b) => {
    if (a.source !== b.source) {
      const order: Record<ConfigSource, number> = { project: 0, user: 1, gemini: 2 }
...
/** Returns extra bin directories that isServerInstalled checks beyond system PATH. */
export function getAdditionalBinPaths(): string[] {
  const cwd = process.cwd()
  const configDir = getGeminiConfigDir({ binary: "gemini" })
  const dataDir = join(getDataDir(), "gemini")
  return [
...
export function isServerInstalled(command: string[]): boolean {
  ...
  for (const base of getAdditionalBinPaths()) {
    for (const suffix of exts) {
  ...
  // Runtime wrappers (bun/node) are always available in oh-my-gemini context
  if (cmd === "bun" || cmd === "node") {
    return true
  }
...
export function getConfigPaths_(): { project: string; user: string; gemini: string } {
