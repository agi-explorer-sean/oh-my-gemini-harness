...
async function formatTaskResult(
  ...
  if (task.type === 'parallel_exec') {
    return await formatParallelReport(task as CompositeBackgroundTask, manager);
  }
...
async function formatParallelReport(
  task: CompositeBackgroundTask,
  manager: BackgroundOutputManager,
): Promise<string> {
  const coordinator = (manager as any).parallelExecs?.get(task.id);
  const results = coordinator
    ? coordinator.getResults()
    : task.parallelResults ?? [];

  if (results.length === 0) {
    return `# Parallel Execution Report: ${task.description}
Status: ${task.status.toUpperCase()}
(No detailed results available)

Use parallel_status(id="${task.id}") for real-time progress.`;
  }

  const completed = results.filter((r: any) => r.status === 'completed');
  const failed = results.filter((r: any) => r.status === 'failed');

  let report = `# Parallel Execution Report: ${task.description}
...
`;
  ...
  report += `| Task | Status | Output/Error |
|------|--------|--------------|
`;
  for (const res of results) {
    const detail =
      res.error ||
      (res.output ? truncateText(res.output, 100).replace(/\n/g, ' ') : '-');
    report += `| ${res.description} | ${res.status.toUpperCase()} | ${detail} |
`;
  }
...
function extractToolResultText(part: FullSessionMessagePart): string[] {
  ...
  if (Array.isArray(part.content)) {
    const blocks = part.content
      .filter(
        (block) =>
          (block.type === 'text' || block.type === 'reasoning') && block.text,
      )
...
async function formatFullSession(
  ...
  const sortedMessages = [...(rawMessages as FullSessionMessage[])].sort(
    (a, b) => {
    ...
    async execute(args: BackgroundOutputArgs) {
      try {
        ...
        if (task.status === 'completed') {
          return await formatTaskResult(task, client, manager);
        }
        ...
        while (Date.now() - startTime < timeoutMs) {
          ...
          if (currentTask.status === 'completed') {
            return await formatTaskResult(currentTask, client, manager);
          }
