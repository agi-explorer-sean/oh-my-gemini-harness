...
// Priority: explicit type > url presence > command presence
...
export class SkillMcpManager {
  ...
  private async createHttpClient(
    ...
    if (config.oauth) {
      ...
      if (tokenData) {
        const existingHeaders = (requestInit.headers ?? {}) as Record<
          string,
          string
        >;
        requestInit.headers = {
  ...
  private async withOperationRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const stepUpHandled = await this.handleStepUpIfNeeded(
          lastError,
          config,
        );
        if (stepUpHandled) {
