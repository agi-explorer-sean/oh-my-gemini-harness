...
// Mock createOpencodeClient to return a mock client
mock.module("../../shared", () => ({
  ...shared,
...
describe("BackgroundManager - Non-blocking Queue Integration", () => {
  ...
  describe("launch() returns immediately without blocking", () => {
    test("should return task with valid status immediately", async () => {
      // given
      const input = {
        description: "Test task",
        prompt: "Do something",
      ...
      // then — processKey runs as microtasks and may transition to "running"
      // before the assertion, so accept both valid initial states
    ...
    test("should return immediately even with concurrency limit", async () => {
      ...
      // then — launch should return quickly (non-blocking)
      expect(endTime - startTime).toBeLessThan(100)
      // First task may already be "running" (slot acquired via microtask),
      // second task should be "pending" (concurrency limit 1)
    ...
    test("should queue multiple tasks without blocking", async () => {
      ...
      // then — tasks should be created without blocking
  ...
  describe("task transitions pending→running when slot available", () => {
    test("should transition first task to running immediately", async () => {
      ...
      // In subprocess mode, sessionID is not set (only in session-based mode)
      // Check directory instead which is always set
      expect(updatedTask?.directory).toBeDefined()
    })
  ...
  describe("manual verification scenario", () => {
    test("should handle 10 tasks with limit 5 returning immediately", async () => {
      ...
      tasks.forEach(task => {
        // processKey runs as microtasks, so some tasks may already
        // be "running" by the time assertions execute
        expect(["pending", "running"]).toContain(task.status)
        expect(task.id).toMatch(/^bg_/)
      })
      ...
      // Verify at most 5 running (concurrency limit), rest pending or queued
