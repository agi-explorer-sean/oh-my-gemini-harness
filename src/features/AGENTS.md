...
├── gemini-skill-loader/      # Skills from 6 directories
...
├── parallel-exec/             # Parallel coordination
├── sisyphus-tasks/             # Task tracking
└── gemini-tasks/               # Task schema/storage - see AGENTS.md
```
...
|------|--------------------------|
| Commands | `.gemini/command/` > `~/.config/gemini/command/` > `.claude/commands/` |
| Skills | `.gemini/skills/` > `~/.config/gemini/skills/` > `.claude/skills/` |
