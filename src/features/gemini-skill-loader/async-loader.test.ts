...
const SKILLS_DIR = join(TEST_DIR, ".gemini", "skills")
...
`
      ...
      // when
      const { getSkillFromPathAsync } = await import("./async-loader")
      const skill = await getSkillFromPathAsync(skillPath, skillDir, "path-skill", "gemini-project")

      // then
      expect(skill).not.toBeNull()
      expect(skill?.name).toBe("path-skill")
      expect(skill?.scope).toBe("gemini-project")
    })
    ...
    it("returns null for invalid path", async () => {
      ...
      // when
      const { getSkillFromPathAsync } = await import("./async-loader")
      const skill = await getSkillFromPathAsync(invalidPath, TEST_DIR, "invalid", "gemini")
    ...
    it("returns null for malformed skill file", async () => {
      ...
      // when
      const { getSkillFromPathAsync } = await import("./async-loader")
      const skill = await getSkillFromPathAsync(malformedPath, SKILLS_DIR, "malformed", "user")
  ...
  describe("loadMcpJsonFromDirAsync", () => {
    it("loads mcp.json with mcpServers format", async () => {
      ...
      // when
      const { getMcpJsonFromDirAsync } = await import("./async-loader")
      const config = await getMcpJsonFromDirAsync(SKILLS_DIR)
