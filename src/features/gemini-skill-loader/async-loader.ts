...
export async function getMcpJsonFromDirAsync(skillDir: string): Promise<SkillMcpConfig | undefined> {
...
export async function getSkillFromPathAsync(
  skillPath: string,
  resolvedPath: string,
  defaultName: string,
  scope: SkillScope
  ...
  try {
    ...
    const mcpJsonMcp = await getMcpJsonFromDirAsync(resolvedPath)
    ...
    const processEntry = async (entry: Dirent): Promise<LoadedSkill | null> => {
      ...
      if (entry.isDirectory() || entry.isSymbolicLink()) {
        ...
        try {
          ...
          return await getSkillFromPathAsync(skillMdPath, resolvedPath, dirName, "gemini-project")
        } catch {
          const namedSkillMdPath = join(resolvedPath, `${dirName}.md`)
          try {
            await readFile(namedSkillMdPath, "utf-8")
            return await getSkillFromPathAsync(namedSkillMdPath, resolvedPath, dirName, "gemini-project")
          } catch {
            return null
          }
        }
      ...
      if (isMarkdownFile(entry)) {
        ...
        return await getSkillFromPathAsync(entryPath, skillsDir, skillName, "gemini-project")
      }
