...
import {NO_AGENT_TEMPLATE} from './templates/no-agent';
...
${STOP_CONTINUATION_TEMPLATE}
  ...
  'no-agent': {
    description:
      '(builtin) Toggle no-agent mode - bypass default agent delegation',
    template: `<command-instruction>
${NO_AGENT_TEMPLATE}
</command-instruction>`,
  },
...
export function getBuiltinCommands(
  ...
  for (const [name, definition] of Object.entries(
    ...
    if (!disabled.has(name as BuiltinCommandName)) {
      const {argumentHint: _argumentHint, ...geminiCompatible} = definition;
      commands[name] = {...geminiCompatible, name} as CommandDefinition;
    }
  }
