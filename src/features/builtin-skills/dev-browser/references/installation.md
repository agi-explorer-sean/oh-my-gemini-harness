...
# For oh-my-gemini: already bundled
# For manual installation:
mkdir -p ~/.config/gemini/skills
cp -r /tmp/dev-browser-skill/skills/dev-browser ~/.config/gemini/skills/dev-browser
...
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.config\gemini\skills"
Copy-Item -Recurse "$env:TEMP\dev-browser-skill\skills\dev-browser" "$env:USERPROFILE\.config\gemini\skills\dev-browser"
...
```bash
cd ~/.config/gemini/skills/dev-browser
npm install
```

**Windows (PowerShell):**
```powershell
cd "$env:USERPROFILE\.config\gemini\skills\dev-browser"
npm install
```
...
**Windows (CMD):**
```cmd
cd %USERPROFILE%\.config\gemini\skills\dev-browser
