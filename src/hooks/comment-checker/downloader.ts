...
const REPO = "agi-explorer-sean/go-claude-code-comment-checker"
...
/**
 * Get the cache directory for oh-my-gemini binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "oh-my-gemini", "bin")
  }
...
/**
 * Get the version from the installed @agi-explorer-sean/comment-checker package.
 */
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[oh-my-gemini] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[oh-my-gemini] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[oh-my-gemini] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[oh-my-gemini] Comment checking disabled.`)
    return null
  }
}
