...
export function detectErrorType(error: unknown): RecoveryErrorType {
  ...
  // Check thinking_block_order BEFORE tool_result_missing (thinking errors contain "tool_use"/"tool_result" in URLs)
  if (
...
function extractToolUseIds(parts: MessagePart[]): string[] {
  return parts
    .filter((p): p is ToolUsePart => p.type === 'tool_use' && !!p.id)
    .map((p) => p.id);
}
...
async function recoverToolResultMissing(
  ...
  try {
    await (client.session.prompt as Function)({
      path: {id: sessionID},
