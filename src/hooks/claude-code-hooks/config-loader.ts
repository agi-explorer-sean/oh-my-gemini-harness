...
import { getGeminiConfigDir } from "../../shared"
...
const USER_CONFIG_PATH = join(getGeminiConfigDir({ binary: "gemini" }), "gemini-cc-plugin.json")

function getProjectConfigPath(): string {
  return join(process.cwd(), ".gemini", "gemini-cc-plugin.json")
}

async function getConfigFromPath(path: string): Promise<PluginExtendedConfig | null> {
  if (!existsSync(path)) {
    return null
  }
...
export async function getPluginExtendedConfig(): Promise<PluginExtendedConfig> {
  const userConfig = await getConfigFromPath(USER_CONFIG_PATH)
  const projectConfig = await getConfigFromPath(getProjectConfigPath())
