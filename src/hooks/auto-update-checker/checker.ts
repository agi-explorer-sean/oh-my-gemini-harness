...
function getConfigPaths(directory: string): string[] {
  ...
  if (process.platform === 'win32') {
    ...
    if (appdataDir) {
      ...
      const alternateConfig = path.join(alternateDir, 'gemini', 'gemini.json');
...
export function getLocalDevPath(directory: string): string | null {
  for (const configPath of getConfigPaths(directory)) {
    try {
      ...
      const config = JSON.parse(stripJsonComments(content)) as GeminiConfig;
...
// Updates a pinned version in the config file's "plugin" array, preserving JSONC formatting
