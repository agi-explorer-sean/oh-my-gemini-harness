...
export function createAutoUpdateCheckerHook(ctx: PluginInput, options: AutoUpdateCheckerOptions = {}) {
  ...
  const getToastMessage = (isUpdate: boolean, latestVersion?: string): string => {
    if (isSisyphusEnabled) {
      return isUpdate
        ? `Sisyphus on steroids is steering Gemini.\nv${latestVersion} available. Restart to apply.`
        : `Sisyphus on steroids is steering Gemini.`
    }
    return isUpdate
      ? `Gemini is now on Steroids. oMgMgMgMgMg...${latestVersion} available. Restart Gemini to apply.`
      : `Gemini is now on Steroids. oMgMgMgMgMg...`
  }
  ...
  await ctx.client.tui
    .showToast({
      body: {
        ...
        message: "Run 'gemini models --refresh' or restart Gemini to populate the models cache for optimal agent model selection.",
    ...
    await ctx.client.tui
      .showToast({
        body: {
          ...
          message: "Building provider cache for first time. Restart Gemini for full model filtering.",
...
async function showSpinnerToast(ctx: PluginInput, version: string, message: string): Promise<void> {
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    await ctx.client.tui
      .showToast({
        body: {
          title: `${spinner} OhMyGemini ${version}`,
          message,
          variant: "info" as const,
...
async function showUpdateAvailableToast(
  ...
  await ctx.client.tui
    .showToast({
      body: {
        title: `OhMyGemini ${latestVersion}`,
...
async function showAutoUpdatedToast(ctx: PluginInput, oldVersion: string, newVersion: string): Promise<void> {
  await ctx.client.tui
    .showToast({
      body: {
        title: `OhMyGemini Updated!`,
        message: `v${oldVersion} → v${newVersion}\nRestart Gemini to apply.`,
...
async function showLocalDevToast(ctx: PluginInput, version: string | null, isSisyphusEnabled: boolean): Promise<void> {
  ...
  const message = isSisyphusEnabled
    ...
    : "Running in local development mode. oMgMgMgMgMg..."
