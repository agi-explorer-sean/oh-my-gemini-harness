...
} from '../../third_party/oh-my-opencode/src/hooks/session-notification-utils';
import {
  getMainSessionID,
  subagentSessions,
...
$Notifier = [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('Gemini')
$Notifier.Show($Toast)
`
        .trim()
        .replace(/\n/g, '; ');
...
async function hasIncompleteTodos(
  ...
  try {
    ...
    // session.todo not available in Gemini CLI mode — skip todo check
    return false;
  }
}
