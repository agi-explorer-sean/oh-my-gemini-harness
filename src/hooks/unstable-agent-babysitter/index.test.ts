...
const projectDir = "/Users/yeongyu/local-workspaces/oh-my-gemini"
...
describe("unstable-agent-babysitter hook", () => {
  ...
  test("fires reminder for hung gemini task", async () => {
    // #given
    const mainSessionID = "main-" + Math.random()
    const bgSessionID = "bg-" + Math.random()
    ...
    const backgroundManager = createBackgroundManager([createTask({ parentSessionID: mainSessionID, sessionID: bgSessionID })])
    ...
    // #when
    await hook.event({ event: { type: "session.idle", properties: { sessionID: mainSessionID } } })
  ...
  test("fires reminder for hung minimax task", async () => {
    ...
    const backgroundManager = createBackgroundManager([
      createTask({ parentSessionID: mainSessionID, sessionID: bgSessionID, model: { providerID: "minimax", modelID: "minimax-1" } }),
    ])
    const hook = createUnstableAgentBabysitterHook(ctx, {
      backgroundManager,
      config: { timeout_ms: 120000 },
  ...
  test("respects per-task cooldown", async () => {
    ...
    const promptCalls: Array<{ input: unknown }> = []
    const ctx = createMockPluginInput({
      messagesBySession: { [mainSessionID]: [], "bg-1": [] },
      promptCalls,
    })
    const backgroundManager = createBackgroundManager([createTask({ parentSessionID: mainSessionID })])
    const hook = createUnstableAgentBabysitterHook(ctx, {
      backgroundManager,
      config: { timeout_ms: 120000 },
    })
