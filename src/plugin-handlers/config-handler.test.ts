...
import * as skillLoader from "../features/gemini-skill-loader"
import * as agentLoader from "../features/claude-code-agent-loader"
import * as mcpLoader from "../features/claude-code-mcp-loader"
import * as pluginLoader from "../features/claude-code-plugin-loader"
import * as mcpModule from "../mcp"
import * as shared from "../shared"
import * as configDir from "../shared/gemini-config-dir"
...
beforeEach(() => {
  ...
  spyOn(commandLoader, "loadGeminiGlobalCommands" as any).mockResolvedValue({})
  spyOn(commandLoader, "loadGeminiProjectCommands" as any).mockResolvedValue({})

  spyOn(builtinCommands, "getBuiltinCommands" as any).mockReturnValue({})

  spyOn(skillLoader, "loadUserSkills" as any).mockResolvedValue({})
  spyOn(skillLoader, "loadProjectSkills" as any).mockResolvedValue({})
  spyOn(skillLoader, "loadGeminiGlobalSkills" as any).mockResolvedValue({})
  spyOn(skillLoader, "loadGeminiProjectSkills" as any).mockResolvedValue({})
  ...
  spyOn(skillLoader, "discoverGeminiGlobalSkills" as any).mockResolvedValue([])
  spyOn(skillLoader, "discoverGeminiProjectSkills" as any).mockResolvedValue([])
  ...
  spyOn(shared, "fetchAvailableModels" as any).mockResolvedValue(new Set(["google/gemini-3-pro-preview"]))
  spyOn(shared, "readConnectedProvidersCache" as any).mockReturnValue(null)

  spyOn(configDir, "getGeminiConfigPaths" as any).mockReturnValue({
    global: "/tmp/.config/gemini",
    project: "/tmp/.gemini",
  })

  spyOn(permissionCompat, "migrateAgentConfig" as any).mockImplementation((config: Record<string, unknown>) => config)

  spyOn(modelResolver, "resolveModelWithFallback" as any).mockReturnValue({ model: "google/gemini-3-pro-preview" })
})
...
afterEach(() => {
  ...
  ;(commandLoader.loadGeminiGlobalCommands as any)?.mockRestore?.()
  ;(commandLoader.loadGeminiProjectCommands as any)?.mockRestore?.()
  ;(builtinCommands.getBuiltinCommands as any)?.mockRestore?.()
  ;(skillLoader.loadUserSkills as any)?.mockRestore?.()
  ;(skillLoader.loadProjectSkills as any)?.mockRestore?.()
  ;(skillLoader.loadGeminiGlobalSkills as any)?.mockRestore?.()
  ;(skillLoader.loadGeminiProjectSkills as any)?.mockRestore?.()
  ;(skillLoader.discoverUserClaudeSkills as any)?.mockRestore?.()
  ;(skillLoader.discoverProjectClaudeSkills as any)?.mockRestore?.()
  ;(skillLoader.discoverGeminiGlobalSkills as any)?.mockRestore?.()
  ;(skillLoader.discoverGeminiProjectSkills as any)?.mockRestore?.()
  ...
  ;(configDir.getGeminiConfigPaths as any)?.mockRestore?.()
...
describe("Plan agent demote behavior", () => {
  test("orders core agents as sisyphus -> hephaestus -> prometheus -> atlas", async () => {
    ...
    const pluginConfig: OhMyGeminiConfig = {
      sisyphus_agent: {
